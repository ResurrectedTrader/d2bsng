...
struct GameLoopFixture {
    GameLoopFixture() {
        ...
        d2bs::test::Reset();
        d2bs::runtime::script::ScriptEngine::Instance().Reset();
        d2bs::runtime::script::ScriptEngine::Instance().SetInitialized(true);
...
TEST_CASE_FIXTURE(GameLoopFixture, "Menu->InGame starts default game script") {
    ...
    CHECK(d2bs::runtime::script::ScriptEngine::Instance().StartedScripts().empty());

    s.clientState = GameState::InGame;
    s.playerId = 1;
    GameLoop::Instance().OnSleep(std::chrono::milliseconds{0});

    const auto& started = d2bs::runtime::script::ScriptEngine::Instance().StartedScripts();
    REQUIRE(started.size() == 1);
    CHECK(started[0]->GetMode() == d2bs::runtime::script::ScriptMode::InGame);
...
TEST_CASE_FIXTURE(GameLoopFixture, "InGame->Menu stops in-game scripts and preserves OOG starter") {
    ...
    CHECK(inGameScript->GetMode() == d2bs::runtime::script::ScriptMode::InGame);
...
struct TempIniForGameLoop {
    ...
    TempIniForGameLoop() {
        ...
        auto& cfg = d2bs::core::config::GetAppConfig();
        cfg.store = std::make_unique<d2bs::core::config::IniConfigStore>(path);
    }

    ~TempIniForGameLoop() {
        std::error_code ec;
        std::filesystem::remove(path, ec);
        d2bs::core::config::GetAppConfig().store.reset();
    }
};
...
// the existing [<profileName>] section that runtime::profile::Add already created -
...
TEST_CASE_FIXTURE(GameLoopFixture, "Latch cleared via Switch launches starter on next tick") {
    ...
    CHECK(d2bs::runtime::profile::Switch("p1") == true);

    // Second tick: latch just cleared -> starter launches exactly once.
    GameLoop::Instance().OnSleep(std::chrono::milliseconds{0});
    const auto& started = d2bs::runtime::script::ScriptEngine::Instance().StartedScripts();
    REQUIRE(started.size() == 1);
    CHECK(started[0]->GetMode() == d2bs::runtime::script::ScriptMode::OutOfGame);
...
TEST_CASE_FIXTURE(GameLoopFixture, "CLI launch profile seeds name once - no double launch") {
    ...
    CHECK(d2bs::runtime::profile::Switch("foo") == true);
    ...
    GameLoop::Instance().OnSleep(std::chrono::milliseconds{0});
    CHECK(d2bs::runtime::script::ScriptEngine::Instance().StartedScripts().size() == 1);
}
...
TEST_CASE_FIXTURE(GameLoopFixture, "Mid-session /profile bar reloads overrides, relaunches OOG starter") {
    ...
    CHECK(d2bs::runtime::profile::Switch("bar") == true);

    GameLoop::Instance().OnSleep(std::chrono::milliseconds{0});
    REQUIRE(started.size() == 2);
    CHECK(prevStarter->IsStopped());
    CHECK(started[1]->GetPath().filename() == "bar_starter.dbj");
    CHECK(started[1]->GetMode() == d2bs::runtime::script::ScriptMode::OutOfGame);
}
...
TEST_CASE_FIXTURE(GameLoopFixture, "/profile at InGame swaps game script only") {
    ...
    d2bs::runtime::profile::Add(profA);
    ...
    d2bs::runtime::profile::Add(profB);
    ...
    CHECK(d2bs::runtime::profile::Switch("A") == true);
    GameLoop::Instance().OnSleep(std::chrono::milliseconds{0});
    const auto& started = d2bs::runtime::script::ScriptEngine::Instance().StartedScripts();
    REQUIRE(started.size() == 1);
    CHECK(started[0]->GetMode() == d2bs::runtime::script::ScriptMode::InGame);
    ...
    CHECK(d2bs::runtime::profile::Switch("B") == true);
    GameLoop::Instance().OnSleep(std::chrono::milliseconds{0});
    REQUIRE(started.size() == 2);
    CHECK(aScript->IsStopped());
    CHECK(started[1]->GetMode() == d2bs::runtime::script::ScriptMode::InGame);
    ...
    for (const auto& sc : started) {
        CHECK(sc->GetMode() == d2bs::runtime::script::ScriptMode::InGame);
    }
}
...
TEST_CASE_FIXTURE(GameLoopFixture, "Sticky overrides revert when switching to profile without override") {
    ...
    d2bs::runtime::profile::Add(profileA);
    ...
    d2bs::runtime::profile::Add(profileB);
...
TEST_CASE_FIXTURE(GameLoopFixture, "startAtMenu=false gates initial Menu starter launch") {
    ...
    CHECK(d2bs::runtime::profile::Switch("p") == true);
    GameLoop::Instance().OnSleep(std::chrono::milliseconds{0});
    const auto& started = d2bs::runtime::script::ScriptEngine::Instance().StartedScripts();
    REQUIRE(started.size() == 1);
...
TEST_CASE_FIXTURE(GameLoopFixture, "Console restart observes new paths (BUG A regression)") {
    ...
    s.clientState = GameState::Menu;
    GameLoop::Instance().OnSleep(std::chrono::milliseconds{0});
    d2bs::runtime::script::ScriptEngine::Instance().ResetRestartCount();

    CHECK(d2bs::runtime::profile::Switch("p") == true);
    GameLoop::Instance().OnSleep(std::chrono::milliseconds{0});

    CHECK(d2bs::runtime::script::ScriptEngine::Instance().RestartConsoleCount() == 1);
    ...
    CHECK(d2bs::runtime::script::ScriptEngine::Instance().RestartedConsoleName() == "profile.js");
}
...
TEST_CASE_FIXTURE(GameLoopFixture, "Console script restarts only when consoleScript override changes") {
    ...
    CHECK(d2bs::runtime::profile::Switch("same") == true);
    GameLoop::Instance().OnSleep(std::chrono::milliseconds{0});
    CHECK(d2bs::runtime::script::ScriptEngine::Instance().RestartConsoleCount() == 0);

    // Switch to profile with an override -> console restart fires once.
    CHECK(d2bs::runtime::profile::Switch("diff") == true);
    GameLoop::Instance().OnSleep(std::chrono::milliseconds{0});
    CHECK(d2bs::runtime::script::ScriptEngine::Instance().RestartConsoleCount() == 1);
}
...
TEST_CASE_FIXTURE(GameLoopFixture, "Switch(\"\") is a no-op") {
    ...
    CHECK(d2bs::runtime::profile::Switch("") == false);
    // Name unchanged.
    CHECK(cfg.GetProfileName() == "foo");
    // Latch not cleared.
...
TEST_CASE_FIXTURE(GameLoopFixture, "Switch(\"nonexistent\") is a no-op") {
    ...
    CHECK(d2bs::runtime::profile::Switch("does_not_exist") == false);
    CHECK(cfg.GetProfileName() == "foo");
    CHECK(cfg.waitForProfile.load() == true);
}
...
TEST_CASE_FIXTURE(GameLoopFixture, "InGame->Menu->InGame preserves OOG starter") {
    ...
    CHECK(started[2]->GetMode() == d2bs::runtime::script::ScriptMode::InGame);
    CHECK(started[2]->GetPath().filename() == "game.dbj");
    CHECK_FALSE(starter->IsStopped());
    CHECK(starter.get() == d2bs::runtime::script::ScriptEngine::Instance().StartedScripts()[0].get());
}

TEST_CASE_FIXTURE(GameLoopFixture, "OnSleep is a no-op pre-Initialize") {
    d2bs::runtime::script::ScriptEngine::Instance().SetInitialized(false);
...
std::shared_ptr<d2bs::runtime::script::Script> EnterGameSession() {
    auto& cfg = d2bs::core::config::GetAppConfig();
    d2bs::config::ScriptPaths paths;
    paths.gameScript = "game.dbj";
    cfg.SetScriptPaths(paths);
    ...
    const auto& started = d2bs::runtime::script::ScriptEngine::Instance().StartedScripts();
    REQUIRE(started.size() == 1);
    REQUIRE(started[0]->GetMode() == d2bs::runtime::script::ScriptMode::InGame);
    return started[0];
}
