...
- `docs/plugy_stash.md` - stash tabs: the game-agnostic `StashTab` contract (`game::GetStashTabs` / `GetStashTabItems` / `Unit::StashTab` / `ClickStashTabSlot` / `StashTabGold`, the `getStashTabs()` / `getStashTabItems()` / `clickStashTab()` / `stashTabGold()` globals and `Unit.stashTab`; per-tab `type` and `gold`, with a stash-wide gold figure attributed to the first tab of its kind), how PlugY implements multi-page stashes on 1.14d (only the active page lives in the inventory; the rest hang off a `PYPlayerData` tail PlugY appends to the player data), the version-allowlisted detection in `backends/lod114d/game/PlugY.cpp`, running PlugY's `Init` from `Bridge::Init` when the manager injected PlugY.dll ahead of d2bs (no PlugY.exe), the blocking swap-in / click / swap-back sequence over PlugY's 0x3A channel that lets click calls reach inactive pages without a script-visible "select", and why the vanilla `getItems()` view is left alone.
