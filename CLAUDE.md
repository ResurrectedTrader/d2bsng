...
- `docs/analytics.md` - anonymous usage analytics (Aptabase): the `Analytics` component, the `session_start` / `profile_active` events (the latter carrying a per-install `profileHash`, never the profile name) and exactly what they do / do not collect, the CI-baked `D2BS_ANALYTICS_KEY` compile-time app key (build-time only, `#ifndef`-defaulted to empty = no-op, same mechanism as `D2BS_VERSION`), the per-launch user-id (`-analyticsuser`/env) and opt-out (`-noanalytics`/`D2BS_ANALYTICS_DISABLE`), the backend-agnostic `features` tag list via `game::GetActiveFeatures()`, the anonymous `installId` (derived per launch, never stored), and the `game::GetAnalyticsLaunchOptions()` / `game::GetBackendVersion()` contract accessors.
