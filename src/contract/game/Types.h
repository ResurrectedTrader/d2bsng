...
struct StashTab {
    ...
    uint32_t index = 0;  // 0-based position within its kind
    StashTabType type = StashTabType::Normal;
    std::string name;  // user-given name; empty when the tab is unnamed
    // The tab whose items live in the player's inventory right now, i.e. the one
    // the plain inventory walk (GetItems / FindInvItem) sees as location Stash.
    bool isActive = false;
    // Gold stored on this tab. Where gold is not tracked per tab it is attributed
    // to the first tab of its kind: the character's stash gold to personal tab 0,
    // a shared pool to shared tab 0, and 0 elsewhere.
    uint32_t gold = 0;
};
