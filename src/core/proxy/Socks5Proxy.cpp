...
std::shared_ptr<spdlog::logger> logger;  // the "core.proxy" logger; created in Install(), null while unhooked
...
bool Handshake(SOCKET s, const ProxyConfig& cfg, const sockaddr_in& dest, std::string_view host) {
    ...
    std::vector request{SOCKS5_VERSION, SOCKS5_CMD_CONNECT, SOCKS5_RSV};
...
int WSAAPI HookedConnect(SOCKET s, const sockaddr* name, int namelen) {
    ...
    // opted out (the framework's script sockets - see proxy/ProxyBypass.h),
...
void Install(std::string_view proxySpec) {
    ...
    logger = utils::GetLogger("core.proxy");

    auto parsed = ParseProxyUrl(proxySpec);
    if (!parsed) {
        logger->error("invalid -proxy '{}' (expected socks5://[user:password@]host:port); proxy disabled", proxySpec);
        return;
    }
