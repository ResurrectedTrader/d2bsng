...
bool HandleMouseMessage(const MSG* msg) {
    ...
    switch (msg->message) {
        ...
        case WM_RBUTTONUP: {
            ...
            return activeHooks.onMouseClick(button, pos, state);
        }
        case WM_MOUSEMOVE:
            if (activeHooks.onMouseMove) {
                activeHooks.onMouseMove(pos);
            }
            break;
        default:
            break;
...
bool HandleKeyMessage(const MSG* msg) {
    ...
    const auto state = isUp ? game::KeyState::Up : game::KeyState::Down;
    return activeHooks.onKeyEvent(static_cast<uint32_t>(msg->wParam), state);
}
...
bool Intercept(MSG* msg) {
    if (!activeHooks.intercept || !activeHooks.intercept(*msg)) {
        return false;
    }
    Neutralize(msg);
    return true;
...
LRESULT CALLBACK SubclassedWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    if (msg == WM_COPYDATA && activeHooks.onIPC) {
        ...
        if (cds != nullptr && cds->lpData != nullptr) {
            ...
            activeHooks.onIPC(mode, payload);
        }
    }
