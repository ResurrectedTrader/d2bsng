...
struct Hooks {
    ...
    std::function<bool(game::ClickButton button, game::Position pos, game::KeyState state)> onMouseClick;

    // A human mouse move over the client area.
    std::function<void(game::Position pos)> onMouseMove;

    // A human key transition (auto-repeats are skipped); return true to block
    // it from reaching the game.
    std::function<bool(uint32_t key, game::KeyState state)> onKeyEvent;
