...
enum class AttachResult {
    ...
    DuplicateBody,    // another slot in this batch covers the same code
...
class Batch {
   ...
   private:
    void RecordBody(const void* body);
    [[nodiscard]] bool HasBody(const void* body) const;

    std::vector<Slot*> attaching_;
    std::vector<Slot*> detaching_;
    std::vector<const void*> bodies_;
    int32_t error_ = 0;
    bool isOpen_ = false;
};
