...
bool IniConfigStore::ProfileExists(const std::string& name) {
    ...
    auto count =
        GetPrivateProfileStringW(nullptr, nullptr, nullptr, sections.data(), sections.size(), widePath.c_str());
    if (count == 0) {
        return false;
    }
...
std::string IniConfigStore::ReadString(const std::string& section, const std::string& key,
    ...
    GetPrivateProfileStringW(wideSection.c_str(), wideKey.c_str(), wideDefault.c_str(), buffer.data(), buffer.size(),
                             widePath.c_str());
