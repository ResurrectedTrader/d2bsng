...
// passed at all.
//
// The buffer is the one GetCommandLineW() returns, which is also what
// RTL_USER_PROCESS_PARAMETERS.CommandLine points at - the copy a remote read of
// the PEB, Task Manager, or WMI's Win32_Process.CommandLine sees. Both are
// covered, on the rare systems where they are not the same storage.
//
// It cannot reach a copy another module's CRT already built (the game's own
// argv array, filled at its startup from its own private copy). That one is
// only readable by something already walking the process's heap, which is a
// different threat from anything that reads a process's command line.
