...
//
// The ANSI command line kernelbase built during process init is separate
// storage holding the same text, so it is cut the same way - otherwise the
// whole line would still be readable through one GetCommandLineA() call.
//
