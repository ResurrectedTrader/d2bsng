...
void Host::SetupLogging() {
    ...
    logger_ = utils::GetLogger("d2bs");
    utils::SetLogLevel("d2bs", spdlog::level::debug);

    spdlog::set_default_logger(logger_);
    // Loggers flush from debug, so this only catches trace, which would
    // otherwise sit in the file buffer until the process exits - and after a
    // crash, that is never.
