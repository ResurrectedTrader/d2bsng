...
class Ref {
   public:
    ...
    ~Ref() { assert(OnOwningThread() && "a script function reference is released on its owning script's thread"); }

    [[nodiscard]] bool IsEmpty() const { return function_.IsEmpty(); }

    // Drop the reference. Owning thread only, as for the destructor.
    void Reset() {
        assert(OnOwningThread() && "a script function reference is released on its owning script's thread");
        function_.Reset();
        isolate_ = nullptr;
    }
