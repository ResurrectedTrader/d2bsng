...
void RegisterMenuFunctions(v8::Isolate* isolate, v8::Local<v8::ObjectTemplate> global) {
    ...
    function::Register(
        isolate, global, "createCharacter", +[](const v8::FunctionCallbackInfo<v8::Value>& args) {
            ...
            if (args.Length() < 2) {
                error::ThrowError(isolate, "createCharacter requires at least 2 arguments");
                return;
            }

            if (!error::CheckIsString(args, 0, "name")) {
                return;
            }

            if (!error::CheckIsNumber(args, 1, "type")) {
                return;
            }
    ...
    function::Register(
        isolate, global, "createGame", +[](const v8::FunctionCallbackInfo<v8::Value>& args) {
            ...
            if (!error::CheckArgCount(args, 1, "createGame")) {
                return;
            }
            ...
            if (diff < 0 || diff > static_cast<int32_t>(game::Difficulty::HighestAvailable)) {
                error::ThrowError(isolate, "Invalid difficulty (must be 0-3)");
                return;
            }

            if (!game::CreateGame(name, pass, static_cast<game::Difficulty>(diff))) {
                error::ThrowError(isolate, "createGame failed");
                return;
            }
        });
    ...
    function::Register(
        isolate, global, "joinGame", +[](const v8::FunctionCallbackInfo<v8::Value>& args) {
            ...
            if (!error::CheckArgCount(args, 1, "joinGame")) {
                return;
            }
            ...
            if (!game::JoinGame(name, pass)) {
                error::ThrowError(isolate, "joinGame failed");
                return;
            }
        });
    ...
    function::Register(
        isolate, global, "addProfile", +[](const v8::FunctionCallbackInfo<v8::Value>& args) {
            ...
            if (args.Length() < 6) {
                error::ThrowError(isolate, "Invalid arguments passed to addProfile");
                return;
            }
