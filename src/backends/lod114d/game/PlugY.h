...
// installed in this process. Detected on first use; a verdict is cached once it
// is final (a missing module is retried, and "not patched yet" only becomes
// final once a player unit exists, by which time PlugY's Init has long run).
bool IsActive();

// IsActive() and the local player's page mirror is populated. False out of game,
// and on Battle.net where PlugY leaves the stash untouched. Takes its own read lock.
bool HasPages();
...
// vanilla item is ever in that state. `item` must be a UNIT_ITEM.
