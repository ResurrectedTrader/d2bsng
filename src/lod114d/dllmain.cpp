...
#include "components/Framework.h"
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ulReasonForCall, LPVOID lpReserved) {
    switch (ulReasonForCall) {
        case DLL_PROCESS_ATTACH:
            ...
            // Both calls are idempotent so the async init path in Framework::Initialize
            ...
            d2bs::Framework::Initialize(hModule);
            break;
        case DLL_PROCESS_DETACH:
            if (lpReserved == nullptr) {
                d2bs::Framework::Shutdown();
            }
            break;
        default:
            break;
