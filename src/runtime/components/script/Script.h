...
class Script : public std::enable_shared_from_this<Script> {
   public:
    ...
    bool ExecuteEvent(const std::shared_ptr<events::BaseEvent>& event);
    ...
    void AddDrawable(std::shared_ptr<drawing::Drawable> drawable);
    ...
    // be torn down.
    void RemoveDrawable(const std::shared_ptr<drawing::Drawable>& drawable, bool fireLeaveEvent = true);
    [[nodiscard]] std::vector<std::shared_ptr<drawing::Drawable>> GetDrawables();
    ...
    void SetDrawableHandler(drawing::Drawable& drawable, DrawableHandler which, v8::Local<v8::Function> handler);
    [[nodiscard]] v8::MaybeLocal<v8::Function> GetDrawableHandler(const drawing::Drawable& drawable,
    ...
    // tearing down.
    bool DispatchDrawableClick(std::shared_ptr<const drawing::Drawable> drawable, game::ClickButton button,
                               game::Point pos);
   ...
   private:
    ...
    utils::profiling::Timeline idle_;
    ScriptMode mode_;
    ...
    std::unordered_map<uint32_t, std::shared_ptr<events::DelayedEvent>> delayedEvents_;
    ...
    std::vector<std::shared_ptr<drawing::Drawable>> drawables_;
    ...
    std::unordered_map<const drawing::Drawable*, DrawableHandlers> drawableHandlers_;
    ...
    std::unique_ptr<inspector::ScriptInspector> inspector_;
};
