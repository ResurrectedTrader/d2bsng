...
`input::Install(hwnd, hooks)` takes an `input::Hooks` table of `std::function`s
mirroring the input entries of `GameCallbacks` - `onMouseClick`, `onMouseMove`,
`onKeyEvent`, `onIPC` - plus the optional `intercept`. Each backend's
`HookManager.cpp` builds it from the active `GameCallbacks` table
(`BuildInputHooks`), so core never sees a backend or the callbacks table itself.
A null table entry leaves the hook empty, which the dispatcher treats as "not
handled".
